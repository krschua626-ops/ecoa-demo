      ...
      <div className="flex border-b border-slate-100 px-5 pt-4 gap-4">
        <button
          ...
          className={`pb-3 text-sm font-medium border-b-2 transition-colors ${
            tab === 'english' ? 'border-[#2470B7] text-[#1B5EA6]' : 'border-transparent text-slate-500 hover:text-slate-700'
          }`}
        >
          English · Approved
        </button>
        <button
          ...
          className={`pb-3 text-sm font-medium border-b-2 transition-colors ${
            tab === 'german' ? 'border-[#2470B7] text-[#1B5EA6]' : 'border-transparent text-slate-500 hover:text-slate-700'
          }`}
        >
          Arabic (IL) · Flagged
        </button>
          ...
          <div className="space-y-1.5">
            {freqOptions.map(opt => (
              <label key={opt} className="flex items-center gap-2.5 text-sm text-slate-700 font-mono">
                ...
                {opt === '1     BOWEL MOVEMENTS AS OR MORE FREQUENT THAN THEY HAVE EVER BEEN' ? <span className="font-semibold text-slate-900 bg-[#EBF3FC] px-1 rounded">{opt}</span> : opt}
              </label>
            ))}
          </div>
        ...
        <div>
          ...
          <div className="space-y-1.5">
            {[
              ...
              <label key={opt} className={`flex items-center gap-2.5 text-xs font-mono ${opt.startsWith('1') ? 'font-semibold text-slate-900 bg-[#EBF3FC] px-2 py-1 rounded' : 'text-slate-700'}`}>
                <span className={`w-4 h-4 rounded-full border-2 flex-shrink-0 ${opt.startsWith('1') ? 'border-[#2470B7] bg-[#2470B7]' : 'border-slate-300'}`}>
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 p-5 h-full">
      ...
      <div className="space-y-4">
        ...
        <div>
          <p className="text-xs font-semibold text-[#2470B7] uppercase tracking-wide mb-1.5">Proposed edit</p>
          <p className="text-sm font-medium text-slate-900 leading-relaxed bg-[#EBF3FC] border border-[#D4E9F8] rounded-lg px-3 py-2">{flag.proposedEdit}</p>
        </div>
        ...
        <div className="w-80 flex-shrink-0 flex flex-col gap-3 overflow-y-auto">
                  ...
                  return (
                    <div
                      ...
                      className={`rounded-xl border cursor-pointer transition-all ${
                        isSelected
                          ? 'border-[#5B9FD4] shadow-sm bg-white'
                    ...
                    >
                      ...
                      <div className="px-4 py-3">
                        ...
                        <p className="text-xs text-[#1B5EA6] font-medium mt-1.5">{flag.proposedEditDisplay}</p>
                      </div>
                        ...
                        <div className="border-t border-slate-100 px-4 py-3 bg-slate-50/80 rounded-b-xl">
                          {isClassifying ? (
                            <div className="flex items-center gap-2 text-slate-500 text-xs py-2">
                              <svg className="w-3.5 h-3.5 animate-spin text-[#2470B7]" fill="none" viewBox="0 0 24 24">
          ...
          {pendingFlags.length === 0 && resolvedFlags.length > 0 && (
            <div className="mt-2">
              <Link
                href="/intake"
                className="block w-full text-center px-4 py-2.5 bg-[#1B5EA6] text-white text-sm font-semibold rounded-lg hover:bg-[#154E8C] transition-colors"
              >
